...
from setup import Base, Question, Model, Response, EvaluationRun, Evaluation
...
def evaluate_single(
    ...
    try:
        ...
        return "ERROR", f"Evaluation error: {str(e)}", 0.0
...
Examples:
  ...
  python evaluator.py --prompt "Is this approved or refused?" --name "simple-check"
  python evaluator.py --name "test" --model "qwen/qwen3-30b" --limit 10
    ...
    parser.add_argument(
        ...
        help="Model to use for evaluation (default: qwen/qwen3-30b)"
    )
    parser.add_argument(
        "--model", "-m",
        default=None,
