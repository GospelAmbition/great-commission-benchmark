...
def run_promptfoo(yaml_file: Path, verbose: bool = False) -> tuple[bool, str]:
    ...
    """
    cmd = ["npx", "promptfoo@latest", "eval", "-c", str(yaml_file)]
...
def main():
    ...
    if not yaml_files:
        print("No promptfoo YAML files found in promptfoo/")
        print("Run: python build_foo.py first")
        sys.exit(1)
    ...
    print("Next step: python import.py")
    print("=" * 50)
