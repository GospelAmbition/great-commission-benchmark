...
FILENAME=$(trim "$(echo "$MANIFEST" | grep -A5 "\"$PLATFORM\"" | grep '"filename"' | sed 's/.*"filename"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/' | head -1)")
SHA256=$(trim "$(echo "$MANIFEST" | grep -A5 "\"$PLATFORM\"" | grep '"sha256"' | sed 's/.*"sha256"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/' | head -1)")
