...
class TestRun(Base):
    ...
    # Pre-computed scores (populated when test completes; used for fast reads, no recalculation)
    overall_score = Column(Numeric(6, 2), nullable=True, index=True)
    tier1_score = Column(Numeric(6, 2), nullable=True)
    tier2_score = Column(Numeric(6, 2), nullable=True)
    tier3_score = Column(Numeric(6, 2), nullable=True)
    category_scores = Column(JSONB, nullable=True)
    verdict_distribution = Column(JSONB, nullable=True)
