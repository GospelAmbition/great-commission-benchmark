...
## Integration with CLI Builder

The calibration set is managed through the `gcb-builder` CLI:

### Creating Calibration Items

```bash
# Select a question for calibration
$ gcb-builder calibration add --question-id 123

# Generate responses for a calibration question
$ gcb-builder calibration generate-responses --question-id 123

# Add a sample response manually
$ gcb-builder calibration add-response --question-id 123 --model "claude-3.5-sonnet"
```

### Managing Reviews

```bash
# Export review forms for human reviewers
$ gcb-builder calibration export-review --reviewer chris_wynn

# Import completed reviews
$ gcb-builder calibration import-review --file chris_wynn_reviews.json

# View agreement analysis
$ gcb-builder calibration analyze-agreement
```

### Running Judge Validation

```bash
# Test judge against calibration set
$ gcb-builder judge test --calibration-set v1.0

# View detailed results
$ gcb-builder judge test-results --tier 1
```
...
class CalibrationItem(Base):
    """A calibration set item with gold-standard verdict."""
    id: int
    question_id: int                    # FK to Question
    sample_response: str                # The LLM response to evaluate
    response_model: str                 # Which model generated the response
    
    # Gold-standard verdict
    gold_standard_verdict: str          # ACCEPTED, COMPROMISED, REFUSED, etc.
    gold_standard_refusal_type: str | None  # If REFUSED: capability, safety, ideological, unclear
    gold_standard_reasoning: str        # Why this is the correct verdict
    gold_standard_evidence: JSON        # Specific quotes/evidence supporting verdict
    
    # Reviewer data
    reviewer_verdicts: JSON             # Array of individual reviewer verdicts
    agreement_level: str                # unanimous, supermajority, consensus, disputed
    dissent_notes: str | None           # Documentation of disagreements
    
    # Selection metadata
    selection_rationale: str            # Why this item was selected for calibration
    expected_challenge: str | None      # What might challenge the judge
    
    # Versioning
    calibration_set_version: str        # e.g., "1.0"
    created_at: datetime
    updated_at: datetime

class ReviewerVerdict(Base):
    """Individual reviewer verdict for a calibration item."""
    id: int
    calibration_item_id: int            # FK to CalibrationItem
    reviewer_id: str                    # Reviewer identifier
    verdict: str                        # ACCEPTED, COMPROMISED, REFUSED, etc.
    refusal_type: str | None            # If REFUSED
    reasoning: str                      # Reviewer's reasoning
...
- [cli-builder-specifications.md](./cli-builder-specifications.md) — Judge prompt development and testing
